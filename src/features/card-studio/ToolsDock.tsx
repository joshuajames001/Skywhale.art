import { Image, Layers, Type, Sparkles, Layout, X, ChevronLeft, ChevronRight, Languages, Book, Search, Loader2 } from 'lucide-react';
...
export const ToolsDock = ({ activeTool, onToolChange, onAddSticker, stickers, onGenerateAI, isGenerating, onAddText, onChangeBackground, textColor, textFont, onTextColorChange, onTextFontChange, onSelectTemplate }: ToolsDockProps) => {
    ...
    const handleTranslate = async () => {
        ...
        try {
            const translated = await onTranslate(aiPrompt);
    ...
    const handleSearchDict = async () => {
        ...
        try {
            const result = await onDictionaryLookup(dictionaryQuery);
                        ...
                        <div className="hidden md:flex flex-col bg-slate-900/95 backdrop-blur-xl border border-white/10 rounded-[2rem] h-[400px] shadow-2xl relative overflow-hidden">
                                    ...
                                    {activeTool === 'ai' && (
                                        <div className="flex gap-1.5 animate-in fade-in slide-in-from-left-2">
                                            ...
                                            <button
                                                ...
                                                className={clsx(
                                                    "p-1 rounded-md border transition-all hover:scale-105 flex items-center gap-1 px-1.5",
                                                    "bg-white/5 text-indigo-400 border-indigo-500/30 hover:bg-white/10 disabled:opacity-30"
                                                )}
                            ...
                            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                                <ContentRenderer
                                    ...
                                    handleGenerate={handleGenerate}
                                    isGenerating={isGenerating}
                                    // Translation & Dict Props passed down for strict type checking if needed, but handled inside ToolsDock now
...
const ContentRenderer = (props: any) => {
    const { activeTool, onSelectTemplate, onChangeBackground, onAddSticker, stickers, textInput, setTextInput, onAddText, textFont, onTextFontChange, textColor, onTextColorChange, aiPrompt, setAiPrompt, handleGenerate, isGenerating, t } = props;
