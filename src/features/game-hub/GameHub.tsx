...
export const GameHub = ({ onClose, imageUrl, initialGame }: GameHubProps) => {
    ...
    const isGameActive = (view === 'game' && imageUrlState) || view === 'memory-game' || (view === 'coloring-game' && imageUrlState);
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[60] bg-cover bg-center bg-no-repeat overflow-hidden"
            style={{ backgroundImage: `url(${GAMEHUB_BG_URL})` }}
        >
            {/* Static overlay — never re-renders */}
            <div className="absolute inset-0 bg-white/35 pointer-events-none" />

            {/* ACTIVE GAME VIEWS */}
            {view === 'game' && imageUrlState && (
            ...
            {view === 'coloring-game' && imageUrlState && (
                <div className="relative z-10 w-full h-full flex items-center justify-center">
            ...
            {!isGameActive && (
                <div className="relative z-10 w-full h-full flex items-center justify-center p-4">
                    <div className="w-full max-w-7xl max-h-[100dvh] md:max-h-[90vh] flex flex-col items-center overflow-y-auto no-scrollbar p-4 sm:p-6">
                        <style>{`
                        ...
                        <div className="flex flex-col items-center w-full gap-8 md:gap-12 relative min-h-0">
                            ...
                            <div className="w-full flex items-start justify-center relative perspective-1000 py-8">
                                ...
                                {view === 'book-select' && (
                                    <BookSelector onBack={goBack} loading={loading} books={books} onSelectBook={handleBookSelect} />
                                )}
                                {view === 'page-select' && (
                                    <PageSelector bookPages={bookPages} onSelectImage={handleImageSelect} />
                                )}
                                {view === 'difficulty' && (
