-- Fix rate limit RPC: off-by-one (< instead of <=) + simpler window calculation
...
DECLARE
    ...
    v_remaining INTEGER;
BEGIN
    -- Calculate window start: align to p_window_hours intervals from midnight
    -- e.g. 6h windows: 00:00, 06:00, 12:00, 18:00
    ...
    -- Fix: use < not <= (20th request is the LAST allowed, 21st is blocked)
